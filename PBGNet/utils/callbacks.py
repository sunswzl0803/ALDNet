...
class EvalCallback():
    ...
    def get_miou_png(self, image):
        ...
        # ---------------------------------------------------------#
        image = cvtColor(image) #(256, 256, 3)
        ...
        with torch.no_grad():
            ...
            pr1,pr2,pr3,pr4,pr5,pr6 = self.net(images)
            pr = pr1
            # print(pr.shape) #torch.Size([1, 2, 256, 256])
            # ---------------------------------------------------#
            #   取出每一个像素点的种类
            # ---------------------------------------------------#
            pr = F.softmax(pr.permute(0, 2, 3, 1), dim=-1).cpu().numpy()
            # print(pr.shape) #(1, 256, 256, 2)
    ...
    def on_epoch_end(self, epoch, model_eval):
        # if epoch <= 100 and self.eval_flag:
        #     self.net = model_eval
        ...
        if epoch % self.period == 0 and self.eval_flag:
            ...
            _, IoUs, PA_Recall, _,Accuracy = compute_mIoU(gt_dir, pred_dir, self.image_ids, self.num_classes, None)  # 执行计算mIoU的函数
            ...
            # PA
            plt.figure()
            plt.plot(self.epoches, self.accs, 'red', linewidth=2, label='PA')

            plt.grid(True)
            plt.xlabel('Epoch')
            plt.ylabel('PA')
            plt.title('Val Pixel Accuracy Curve')
            plt.legend(loc="upper right")
