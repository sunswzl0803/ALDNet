...
import torch
from nets.pdfnet_training import CE_Loss, Dice_loss, Focal_Loss, BCEDice_Loss
...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen,
                  ...
                  save_dir, early_stopping_patience, local_rank=0):
    total_loss = 0
    total_f_score = 0
    ...
    if local_rank == 0:
        ...
        if len(loss_history.val_loss) <= 1 or (val_loss / epoch_step_val) <= min(loss_history.val_loss):
            print('Save best model to best_epoch_weights.pth')
            torch.save(model.state_dict(), os.path.join(save_dir, "best_epoch_weights.pth"))
            best_val_loss = val_loss / epoch_step_val
            early_stopping_counter = 0
        else:
